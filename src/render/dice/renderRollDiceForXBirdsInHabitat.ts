...
export function renderRollDiceForXBirdsInHabitat(result: DiceActivationResult): HTMLElement {
  ...
  `
    ...
    for (const [index, stat] of Object.entries(result.activationStats)) {
        ...
        const plural = index === '1' ? 'bird' : 'birds'
        const title = document.createElement('h4')
        title.innerHTML = `With <strong><i><big>${index}</big></i></strong> ${plural} in habitat`
